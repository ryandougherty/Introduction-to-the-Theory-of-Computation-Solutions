...
%		1.8		%
\item[1.8]Use the construction in the proof of Theorem 1.45 to give diagrams of NFAs recognizing the union of the languages described in
\begin{enumerate}
...
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,initial text={},on grid,auto] 
   ...
   \node[state,accepting] (q_8) [right=of q_7] {$q_8$};
	...
	(q_7) edge [loop below] node {$0$} (q_7)
	      edge node {$1$} (q_8)
	(q_8) edge [loop right] node {$0,1$} (q_8);
\end{tikzpicture}
...
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,initial text={},on grid,auto] 
   ...
   \node[state,accepting] (q_6) [below right=of q_0] {$q_6$};
   \node[state,accepting] (q_7) [right=of q_6] {$q_7$};
   \node[state,accepting] (q_8) [right=of q_7] {$q_8$};
   \node[state] (q_9) [right=of q_8] {$q_9$};
    \path[->] 
    (q_0) edge node {$\varepsilon$} (q_1)
          edge node {$\varepsilon$} (q_6)
    ...
    (q_4) edge [bend right=40] node {$0$} (q_2)
          edge node {$1$} (q_5)
    (q_5) edge [loop right] node {$0,1$} (q_5)
    (q_6) edge [loop below] node {$0$} (q_6)
    	  edge [bend left=20] node {$1$} (q_7)
   	(q_7) edge [bend left=20] node {$0$} (q_6)
   		  edge node {$1$} (q_8)
   	(q_8) edge node {$0$} (q_9)
   		  edge [loop below] node {$1$} (q_8)
   	(q_9) edge [loop right] node {$0,1$} (q_9);
\end{tikzpicture}
\end{enumerate}
